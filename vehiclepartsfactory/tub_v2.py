...
# class TubWriter_Original(object):
#     def __init__(self, base_path, inputs=[], types=[], metadata=[],
#                  max_catalog_len=1000):
                 
class TubWriter(factory.Part):
    """
    A Donkey part, which can write records to the datastore.
    """
    def __init__(self, cfg):
        super().__init__(loop_time = 1/cfg.DRIVE_LOOP_HZ)
        self.tub = Tub(cfg.DATA_PATH, cfg.DATATUB_INPUTS, cfg.DATATUB_TYPES, cfg.DATATUB_METADATA, cfg.DATATUB_MAX_CATALOG_LEN)
        self.inputs = cfg.DATATUB_INPUTS

        self.run_part = False   # run if database recording value is True      
        self.datalist = []
    ...
    def read_from_bus(self):
        self.run_part = self.data_bus.read('recording')
        if self.run_part is not None:
            if self.run_part:
                self.datalist = self.data_bus.readlist(self.inputs)
        
    def write_to_bus(self):
        self.data_bus.write('tub/num_records', int, self.current_index)
        
    def operate(self):
        if self.datalist is not None:
            record = dict(zip(self.tub.inputs, self.datalist))
            self.tub.write_record(record)
            self.current_index = self.tub.manifest.current_index
...
# class TubWriter(TubWriter):
#     def __init__(self, cfg):
#        super(TubWriter_Original, self).__init__(base_path=cfg.DATA_PATH, inputs=cfg.DATATUB_INPUTS, types=cfg.DATATUB_TYPES)
#        
#        # vehicle parameters 
#        self.inputs = cfg.DATATUB_INPUTS
#        self.outputs = ['tub/num_records']
#        self.threaded = False
#        self.run_condition = 'recording'